...
public class Utils {
    ...
    //Flip alone diagonal
    //Diagonal Flip. The data size was originally h*w*stride, but after flipping it becomes w*h*stride
    ...
    //src is converted to two-dimensional and stored in dst
    ...
    //src is converted to three-dimensional and stored in dst
    public static void expand(float[] src,float[][][] dst){
        int idx=0;
        for (int y=0;y<dst.length;y++)
            for (int x=0;x<dst[0].length;x++)
    ...
    //The rect is extended by pixels pixels up, down, left, and right
